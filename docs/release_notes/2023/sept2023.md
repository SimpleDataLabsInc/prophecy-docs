...
We are excited to introduce the [Package Hub](/engineers/package-hub) with Prophecy version 3.2. This feature allows platform leads to create and share standardized pipeline components, including [custom gems](/engineers/gem-builder), subgraphs, datasets, pipelines, and [more](/engineers/package-hub). For instance, you can now ensure consistent data encryption in line with your company's policies. Simply create a new Project, add the required custom encryption gems and components, and publish it as a Package. Other teams can easily add this as a dependency and utilize the components from the Package.
