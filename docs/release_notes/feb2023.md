...
For creating a [Livy Fabric](../concepts/fabrics/create-a-fabric#livy) previously the Spark and Scala versions were optional and had default values 2.3.0 and 2.11. With this release these fields are made mandatory, and users will need to update their Fabric configurations with correct values. This is because the initialisation is different for spark2 and spark3 and we need to initialise based on Spark version provided. So, It's not valid to assume spark2 by default for the user.
