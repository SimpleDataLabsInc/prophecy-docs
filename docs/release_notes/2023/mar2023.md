...
Please see [here](/docs/extensibility/package-hub/package-hub.md) for more details on this Feature.
...
A User can now change the Data Sampling [Interims](docs/Spark/execution/execution.md#interactive-execution) for Interactive Runs of Configured pipelines(Read-Only mode from Dependencies).
Please see the below video for an example.
