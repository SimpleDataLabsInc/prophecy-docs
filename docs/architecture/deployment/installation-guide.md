...
CRDs installation is a must before rest of the components could be installed. Note that only this operation requires cluster-wide role.
- **Persistent Storage with Dynamic Provisioning:** Few Prophecy services require persistent storage. With Prophecy Operator not expecting cluster-wide role for installation,
it becomes necessary to have dynamic provisioning enabled for persistent volumes (e.g. gp2)  
- **Ingress Controller:** To expose few services externally on a hostname/FQDN, Prophecy installs its own managed nginx ingress controller. 
If the kubernetes cluster already has one running, we can leverage that. Note that Nginx controller service is exposed via a LoadBalancer type 
service and hence the creation of external Loadbalancer should be permissible within the cluster. 
- **Hostname Resolution & SSL Certificate:** Installed Prophecy services can either be exposed on Prophecy's domain **\*.cloud.prophecy.io** or 
customer owned domain. For Prophecy's domain, we are responsible for ensuring resolution of hostname and managing SSL certs (issued by Let's Encrypt) via our 
Controlcenter service hosted in our own infrastructure. For customer owned domain, customer is responsible for managing SSL certs and 
hostname resolution (by making entries in DNS zone).
- Connectivity from K8s cluster to **Spark Environment** (Databricks, Livy etc.) and vice-versa.


### Installation Instructions:

As mentioned above, Prophecy is installed via helm chart whose values can be tweaked as below based on the requirement.
...
| **athena.controlcenter.disabled** | bool   | If false, then different events will be published to controlcenter by athena from customer's k8s cluster regarding health of Prophecy services for better support. Also, SSL certificate and DNS resolution is managed by Prophecy provided the customer uses Prophecy's domain.  If true, then customer needs to provide SSL cert and make sure that `prophecy.wildcardCert.useExternal` is set to `true`.           |    false       | 
| **athena.isDarkCluster**   | bool     | Set to true if your k8s cluster is a dark cluster and has no internet connectivity. In this case, helm charts won't be pulled from Prophecy's public helm repository and will be packaged in docker image of Athena (Prophecy's management service) | false |
...
| athena.enableSignup    | bool     | Set this to true if Signup flow is to be enabled in Prophecy setup. If this parameter is set to true, make sure that SMTP ports are open for k8s cluster so that verification e-mail can be sent during signup process. | false |
...
`Note: Parameters marked with asterisk(*) are mandatory.`  

### Sample Helm commands for different usecases

#### When certificate is provided by Prophecy
...
#### When certificate is provided by Customer and there is no internet connectivity
...
This is the minimum configuration for test/poc install. Prophecy can help finding the right size for each 
