...
Project tests are custom data tests that check for a passing condition, such as checking that the given query doesn’t return any unexpected rows. They are based on [dbt singular tests](https://docs.getdbt.com/docs/build/data-tests#singular-data-tests).

Project tests perform a SQL query that checks the executed result of your project against a passing condition to see if it's successful. This is done by checking whether a particular table, generated by combining one or more database objects from your project, meets a certain condition. Each project can have multiple tests.

By default, the condition for a project test verifies that the given query does not return any failing rows.

## Set up a test

Project tests share the same canvas as models. You can set them up the same way you would develop and run a model.
...
The job runs automatically. You can see the job status by clicking **Detail**.
