...
Previously, A user was asked to enter a unique team when signing up for Prophecy. To improve the sign-up experience, this requirement has been removed
Now the user can just sign up by giving their Name, Email and Company name. They can be added to a team by Admin after signing up.
...
A User editing Prophecy Pipelines in multiple tabs can now detach the cluster Attached to a Pipeline in another tab and attach the current Pipeline by clicking the same cluster.
They will see this warning before the cluster is detached in previous tab.
![Attach-cluster](img/Attach-cluster1.png)

If there were running Jobs in another tab, they will be prompted to cancel the existing Jobs before switching.
...
Please note: this will only appear if the Job is enabled and released.
...
Improved the cluster restart behaviour when adding a new Pipeline dependency. When a new Pipeline dependency is added, cluster will be detached and prompt for a restart as shown below. A small subset of users were experiencing cluster restart loops when using certain dependencies and this has been addressed
