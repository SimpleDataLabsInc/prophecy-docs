...
The PBT cli can be used to build, test and deploy projects created by Prophecy that are present in your local filesystem. Make sure the **DATABRICKS_URL** and **DATABRICKS_TOKEN** environment variables are set appropriately pointing to your Databricks workspace before running any PBT commands.
...
Running unit tests requires **FABRIC_NAME** environment variable to be set. This will be used to pick the correct configuration for running the unit tests.
