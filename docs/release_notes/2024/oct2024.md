...
- **Limit data preview in interims**: There is a new global level flag that admins can use to disable data sampling for a given Fabric. This flag overrides the Pipeline level data sampling settings. When disabled, users won't be able to see production data in the interims when they run the Pipeline.
