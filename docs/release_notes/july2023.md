...
:::info
Note: Existing Livy users who had Execution Metrics enabled before this release may experience failures in their pipelines if above tables are not created. To mitigate this issue, users can either:
...
Users can now select all-purpose Databricks clusters for testing their Jobs. This can be useful for quickly testing changes to Job without affecting the production environment.
...
- Support for [BranchPythonOperator](https://airflow.apache.org/docs/apache-airflow/2.4.3/_api/airflow/operators/python/index.html#airflow.operators.python.BranchPythonOperator): This allows User to “branch” or follow a path following the execution of this task.
...
More details on this feature can be found [here](/docs/low-code-spark/gems/conditional-execution.md).
...
Please read more about this feature [here](/docs/concepts/fabrics/create_a_fabric.md#databricks).
...
We have added In place Warnings and Diagnostics if Users are using plain text Passwords in Gems. Read about the recommended usage [here](/docs/low-code-spark/best-practices/use-dbx-secret.md)
