...
## 2.8.0.0 (February 28, 2023)

- Prophecy Python libs version: 1.4.2
- Prophecy Scala libs version: 7.0.7

### Updates {#UpdatesRelease280}

#### No team required on sign up

Previously, A user was asked to enter a unique team when signing up for Prophecy. To improve the sign-up experience, requirement of providing a Team is now removed.  
Now the user can just sign up by giving his Name, Email and Company name. He can be added to a team by Admin post sign up.

![New_Sign_up](img/new_sign_up.png)

#### Sorting on tables

All metadata tables will now have options to sort on the columns. By default, the entities in table will be sorted on Modified at or Created At time in decreasing order.

![Sorting](img/Sorting.gif)

#### Use a Cluster in Current tab

A User editing Prophecy Pipelines in multiple tabs, can now detach the cluster Attached to a Pipeline in another tab, and attach to current Pipeline by clicking the same cluster.
He will see a warning as below before the cluster is detached in previous tab.
![Attach-cluster](img/Attach-cluster1.png)

If there were running Jobs in another tab, he would be prompted to cancel the existing Jobs before switching.
![Attach-cluster-warning](img/Attach_cluster_job_warning.png)

#### Link to Databricks Workflow in Released Prophecy Jobs

In Job editor view, Added the link to the corresponding Databricks Workflow. Please note, this will only come if the Job is enabled and released.

![Databricks-link](img/Databricks_link.png)

#### Create Pipelines and Jobs directly from Project Browser

Previously user had to navigate out of Pipeline editor to add a new Job or Pipeline. Now user can do it via the same screen and shown below. Please note, the Pipelines and Jobs can only be created in same Project and Git branch using this option.

![Create-pipeline](img/create_pipeline.gif)

#### Alert user on Databricks Token Expiry

If Databricks token is expired or is invalid, user will now see a prompt as below to update his Databricks token. This will be prompted on Jobs editor and Pipeline editor when attached to expired Fabric.
...
Improved the cluster restart behaviour when adding a new Pipeline dependency. When a new Pipeline dependency is added, cluster will be detached and prompt for a restart as shown below. When restarted, clusters were getting stuck in a restart loop which is now fixed.
...
### Behaviour Changes {#BehaviorChangesRelease27}
